...
SONAsubID <- c(501:503,505,507,508,510:512,514:532,534,537,539:542,544:550,553:558,560,561,563:565)
allSubID <- SONAsubID
...
# ===== Exp 4 dprime plot: points + mean + SE + aligned subject lines =====
...
# ------------------------------------------------------------
# 2) Spacing controls: original 3-condition spacing
# ------------------------------------------------------------
set.seed(123)
...
plot_x4 <- base_x4 %>%
  left_join(subj_offsets4, by = c("participant", "nam_unn")) %>%
  mutate(
...
SE_d4_d_x <- SE_d4_d %>%
  mutate(
    nam_unn = factor(nam_unn, levels = levels(d4$nam_unn)),
    rs_rt_n = factor(rs_rt_n, levels = levels(d4$rs_rt_n)),
    stim_center = ifelse(
      nam_unn == "Meaningful",
      meaningful_center,
      abstract_center
...
p4_dprime <- ggplot() +
  ...
  geom_line(
    data = plot_x4 %>% arrange(nam_unn, participant, rs_rt_n),
  ...
  xlab("Stimulus Type") +
  ggtitle("Memory Performance Exp. 4") +
...
# dprime
t.test(wide_4$dprime_RS, wide_4$dprime_RT, paired = TRUE)
t.test(wide_4$dprime_RT, wide_4$dprime_N,  paired = TRUE)
t.test(wide_4$dprime_RS, wide_4$dprime_N,  paired = TRUE)

# Hits
#t.test(wide_4$Hits_RS, wide_4$Hits_RT, paired = TRUE)
#t.test(wide_4$Hits_RT, wide_4$Hits_N,  paired = TRUE)
#t.test(wide_4$Hits_RS, wide_4$Hits_N,  paired = TRUE)

# False alarms
t.test(wide_4$FAs_RS, wide_4$FAs_RT, paired = TRUE)
t.test(wide_4$FAs_RT, wide_4$FAs_N,  paired = TRUE)
t.test(wide_4$FAs_RS, wide_4$FAs_N,  paired = TRUE)


# =========================
# MISSING RESPONSE SUMMARY: EXP 4
# =========================
library(dplyr)
library(ggplot2)
...
for (s in 1:length(allSubID)) {
  ...
  fName <- paste(
    'C:/Users/jgove/OneDrive/Desktop/Nameable and Unnameable Objects/data shaping/multiple/participant',
    allSubID[s],
    '.csv',
    sep = ""
  )
...
ggplot(missing_summary, aes(x = n_missing)) +
  ...
  ggtitle("Exp. 4 Missing Responses by Participant") +
  theme_bw() +
  theme(
...
# =========================================================
# COMMON RECODING (do once)
# =========================================================
dat_exp4 <- dprimeTable %>%
  mutate(
...
print(anova_dprime_res)  # includes Mauchly + corrections
...
eff_posthoc_dprime <- anova_dprime_dat %>%
  group_by(StimType) %>%
  cohens_d(dprime ~ StudyCond, paired = TRUE)
...
eff_posthoc_hits <- anova_hits_dat %>%
  group_by(StimType) %>%
  cohens_d(Hits ~ StudyCond, paired = TRUE)
...
eff_posthoc_fa <- anova_fa_dat %>%
  group_by(StimType) %>%
  cohens_d(FAs ~ StudyCond, paired = TRUE)
...
fa_top4 <- max(
  plot_x4$FAs,
  SE_d4_fa_x$FAs + SE_d4_fa_x$se,
  na.rm = TRUE
) * 1.05

fa_bot4 <- min(
  0,
  min(plot_x4$FAs, na.rm = TRUE)
)
...
SE_dprime_long4 <- summarySE(dprime_long4, measurevar="dprime", groupvars=c("nam_unn","contrast"))
SE_hits_long4   <- summarySE(Hits_long4,   measurevar="Hits",   groupvars=c("nam_unn","contrast"))
SE_fas_long4    <- summarySE(FAs_long4,    measurevar="FAs",    groupvars=c("nam_unn","contrast"))
...
SE_minus_d4 <- SE_dprime_long4 %>%
  mutate(
    stim_num = as.numeric(nam_unn),
    cond_num = as.numeric(contrast),
    x_mean   = stim_num + (cond_num - mean(unique(cond_num))) * spacing_factor
  )
SE_minus_h4 <- SE_hits_long4 %>%
  mutate(
    stim_num = as.numeric(nam_unn),
    cond_num = as.numeric(contrast),
    x_mean   = stim_num + (cond_num - mean(unique(cond_num))) * spacing_factor
  )
SE_minus_fa4 <- SE_fas_long4 %>%
  mutate(
    stim_num = as.numeric(nam_unn),
    cond_num = as.numeric(contrast),
    x_mean   = stim_num + (cond_num - mean(unique(cond_num))) * spacing_factor
...
p4_minus_d <- ggplot() +
  geom_point(data=plot_minus_d4, aes(x=x_plot, y=dprime, color=contrast), size=3, alpha=0.7) +
  geom_line(data=plot_minus_d4 %>% arrange(nam_unn, participant, contrast),
            aes(x=x_plot, y=dprime, group=interaction(participant, nam_unn)),
            alpha=0.35, color="grey") +
  geom_point(data=SE_minus_d4, aes(x=x_mean, y=dprime),
             inherit.aes=FALSE, shape=18, size=4, color="black") +
  geom_errorbar(data=SE_minus_d4, aes(x=x_mean, ymin=dprime-se, ymax=dprime+se),
                inherit.aes=FALSE, width=0.12, color="black") +
  scale_x_continuous(breaks=1:length(levels(d4$nam_unn)), labels=stim_labels) +
  scale_color_manual(name="Contrast", values=contrast_cols) +
  ylab("d' (Condition - Novel)") +
  ggtitle("Memory Performance (Minus Novel) Exp 4") +
  theme_bw()
print(p4_minus_d)

# ---- Plot B2: Hits minus Novel ----
p4_minus_h <- ggplot() +
  geom_point(data=plot_minus_h4, aes(x=x_plot, y=Hits, color=contrast), size=3, alpha=0.7) +
  geom_line(data=plot_minus_h4 %>% arrange(nam_unn, participant, contrast),
            aes(x=x_plot, y=Hits, group=interaction(participant, nam_unn)),
            alpha=0.35, color="grey") +
  geom_point(data=SE_minus_h4, aes(x=x_mean, y=Hits),
             inherit.aes=FALSE, shape=18, size=4, color="black") +
  geom_errorbar(data=SE_minus_h4, aes(x=x_mean, ymin=Hits-se, ymax=Hits+se),
                inherit.aes=FALSE, width=0.12, color="black") +
  scale_x_continuous(breaks=1:length(levels(d4$nam_unn)), labels=stim_labels) +
  scale_color_manual(name="Contrast", values=contrast_cols) +
  ylab("Hits (Condition - Novel)") +
  ggtitle("Hits (Minus Novel) Exp 4") +
  theme_bw()
print(p4_minus_h)

# ---- Plot B3: FAs minus Novel (auto ylim) ----
fa_top4m <- max(plot_minus_fa4$FAs, SE_minus_fa4$FAs + SE_minus_fa4$se, na.rm=TRUE) * 1.05
fa_bot4m <- min(0, min(plot_minus_fa4$FAs, na.rm=TRUE))

p4_minus_fa <- ggplot() +
  geom_point(data=plot_minus_fa4, aes(x=x_plot, y=FAs, color=contrast), size=3, alpha=0.7) +
  geom_line(data=plot_minus_fa4 %>% arrange(nam_unn, participant, contrast),
            aes(x=x_plot, y=FAs, group=interaction(participant, nam_unn)),
            alpha=0.35, color="grey") +
  geom_point(data=SE_minus_fa4, aes(x=x_mean, y=FAs),
             inherit.aes=FALSE, shape=18, size=4, color="black") +
  geom_errorbar(data=SE_minus_fa4, aes(x=x_mean, ymin=FAs-se, ymax=FAs+se),
                inherit.aes=FALSE, width=0.12, color="black") +
  scale_x_continuous(breaks=1:length(levels(d4$nam_unn)), labels=stim_labels) +
  scale_color_manual(name="Contrast", values=contrast_cols) +
  coord_cartesian(ylim=c(fa_bot4m, fa_top4m)) +
  ylab("FAs (Condition - Novel)") +
  ggtitle("FAs (Minus Novel) Exp 4") +
  theme_bw()
print(p4_minus_fa)
...
ggplot(data = SE_FA,
  ...
  geom_errorbar(aes(ymax = FAs + se, ymin = FAs - se),
                position = position_dodge(.9), color = "black", width = .2) +
  scale_x_discrete("Stimulus Type", labels = c("Nameable", "Abstract")) + 
