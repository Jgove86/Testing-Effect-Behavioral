...
SONAsubID <- c(800:803,805,808:811,814:818,820:829,831:836,838,842:848,851:854,856:860,862,864:867)
allSubID <- SONAsubID
...
for (s in 1:length(allSubID)) {
  fName <- paste('C:/Users/jgove/OneDrive/Desktop/Nameable and Unnameable Objects/data shaping/exemplars/participant', allSubID[s], '.csv', sep = "")
...
# ===== Exp 6 dprime plot: points + mean + SE + aligned subject lines =====
...
d6 <- dprimeTable %>%
  ...
  filter(is.finite(dprime), is.finite(Hits), is.finite(FAs))
...
plot_x6 <- base_x6 %>%
  left_join(subj_offsets6, by = c("participant", "nam_unn")) %>%
  mutate(
...
SE_d6_d_x <- SE_d6_d %>%
  mutate(
    nam_unn = factor(nam_unn, levels = levels(d6$nam_unn)),
    rs_rt_n = factor(rs_rt_n, levels = levels(d6$rs_rt_n)),
    stim_center = ifelse(
      nam_unn == "Meaningful",
      meaningful_center,
      abstract_center
...
p6_dprime <- ggplot() +
  ...
  geom_line(
    data = plot_x6 %>% arrange(nam_unn, participant, rs_rt_n),
  ...
  xlab("Stimulus Type") +
  ggtitle("Memory Performance Exp. 6") +
...
# dprime
t.test(wide_6$dprime_RS, wide_6$dprime_RT, paired = TRUE)
t.test(wide_6$dprime_RT, wide_6$dprime_N,  paired = TRUE)
t.test(wide_6$dprime_RS, wide_6$dprime_N,  paired = TRUE)

# Hits
t.test(wide_6$Hits_RS, wide_6$Hits_RT, paired = TRUE)
t.test(wide_6$Hits_RT, wide_6$Hits_N,  paired = TRUE)
t.test(wide_6$Hits_RS, wide_6$Hits_N,  paired = TRUE)

# False alarms
t.test(wide_6$FAs_RS, wide_6$FAs_RT, paired = TRUE)
t.test(wide_6$FAs_RT, wide_6$FAs_N,  paired = TRUE)
t.test(wide_6$FAs_RS, wide_6$FAs_N,  paired = TRUE)

# Testing effect = Retrieval Practice minus Restudy, separately by stim type
testing_effect_6 <- dprimeTable %>%
...
testing_effect_desc_6 <- testing_effect_6 %>%
...
testing_effect_desc_6

testing_effect_wide_6 <- testing_effect_6 %>%
  tidyr::pivot_wider(
    id_cols = participant,
    names_from = nam_unn,
...
colnames(testing_effect_wide_6)

# If labels are nam/unn:
t.test(
  testing_effect_wide_6$unn,
  testing_effect_wide_6$nam,
  paired = TRUE
)

# =========================
# MISSING RESPONSE SUMMARY: EXP 6
# =========================
library(dplyr)
library(ggplot2)
...
for (s in 1:length(allSubID)) {
  ...
  fName <- paste(
    'C:/Users/jgove/OneDrive/Desktop/Nameable and Unnameable Objects/data shaping/exemplars/participant',
    allSubID[s],
    '.csv',
    sep = ""
  )
  ...
  # missing responses = NA or "None" in key_resp_14.keys
  n_missing <- sum(
    trial_rows &
      (is.na(dat$key_resp_14.keys) | dat$key_resp_14.keys == "None")
  )
...
ggplot(missing_summary, aes(x = n_missing)) +
  ...
  ggtitle("Exp. 6 Missing Responses by Participant") +
  theme_bw() +
  theme(
...
# ---- Summary tables for HITS and FAs ----
...
# ---- Plot A2: HITS ----
# ---- Create HITS and FAs summary x-position tables ----
...
SE_d6_h_x <- SE_d6_h %>%
  mutate(
    nam_unn = factor(nam_unn, levels = levels(d6$nam_unn)),
    rs_rt_n = factor(rs_rt_n, levels = levels(d6$rs_rt_n)),
    stim_center = ifelse(nam_unn == "Meaningful", meaningful_center, abstract_center),
    cond_num = as.numeric(rs_rt_n),
    x_mean = stim_center + (cond_num - 2) * spacing_factor
  )
...
fa_top6 <- max(
  plot_x6$FAs,
  SE_d6_fa_x$FAs + SE_d6_fa_x$se,
  na.rm = TRUE
) * 1.05

fa_bot6 <- min(
  0,
  min(plot_x6$FAs, na.rm = TRUE)
)
...
p6_fas <- ggplot() +
  ...
  coord_cartesian(ylim = c(fa_bot6, fa_top6)) +
...
# ------------------------------------------------------------
# B) MINUS NOVEL (Restudy-Novel, Retrieval-Novel): d', Hits, FAs
# ------------------------------------------------------------
wide6 <- d6 %>%
...
SE_dprime_long6 <- summarySE(dprime_long6, measurevar="dprime", groupvars=c("nam_unn","contrast"))
SE_hits_long6   <- summarySE(Hits_long6,   measurevar="Hits",   groupvars=c("nam_unn","contrast"))
SE_fas_long6    <- summarySE(FAs_long6,    measurevar="FAs",    groupvars=c("nam_unn","contrast"))

contrast_cols <- c("Restudy - Novel"=cbbPalette[2], "Retrieval - Novel"=cbbPalette[8])

# x positions + constant subject jitter (shared across the 3 minus-novel plots)
base_minus6 <- dprime_long6 %>%
  mutate(
    stim_num = as.numeric(nam_unn),
    cond_num = as.numeric(contrast),
    x_mean   = stim_num + (cond_num - mean(unique(cond_num))) * spacing_factor
  )
off_minus6 <- base_minus6 %>%
  distinct(participant, nam_unn) %>%
  mutate(subj_jit = jitter(rep(0, n()), amount = jitter_amount))

plot_minus_d6 <- base_minus6 %>%
  left_join(off_minus6, by=c("participant","nam_unn")) %>%
  mutate(x_plot = x_mean + subj_jit)

plot_minus_h6 <- Hits_long6 %>%
  mutate(
    stim_num = as.numeric(nam_unn),
    cond_num = as.numeric(contrast),
    x_mean   = stim_num + (cond_num - mean(unique(cond_num))) * spacing_factor
...
SE_minus_d6 <- SE_dprime_long6 %>%
  mutate(
    stim_num = as.numeric(nam_unn),
    cond_num = as.numeric(contrast),
    x_mean   = stim_num + (cond_num - mean(unique(cond_num))) * spacing_factor
  )
SE_minus_h6 <- SE_hits_long6 %>%
  mutate(
    stim_num = as.numeric(nam_unn),
    cond_num = as.numeric(contrast),
    x_mean   = stim_num + (cond_num - mean(unique(cond_num))) * spacing_factor
  )
SE_minus_fa6 <- SE_fas_long6 %>%
  mutate(
    stim_num = as.numeric(nam_unn),
    cond_num = as.numeric(contrast),
    x_mean   = stim_num + (cond_num - mean(unique(cond_num))) * spacing_factor
...
p6_minus_d <- ggplot() +
  geom_point(data=plot_minus_d6, aes(x=x_plot, y=dprime, color=contrast), size=3, alpha=0.7) +
  geom_line(data=plot_minus_d6 %>% arrange(nam_unn, participant, contrast),
            aes(x=x_plot, y=dprime, group=interaction(participant, nam_unn)),
            alpha=0.35, color="grey") +
  geom_point(data=SE_minus_d6, aes(x=x_mean, y=dprime),
             inherit.aes=FALSE, shape=18, size=4, color="black") +
  geom_errorbar(data=SE_minus_d6, aes(x=x_mean, ymin=dprime-se, ymax=dprime+se),
                inherit.aes=FALSE, width=0.12, color="black") +
  scale_x_continuous(breaks=1:length(levels(d6$nam_unn)), labels=stim_labels) +
  scale_color_manual(name="Contrast", values=contrast_cols) +
  ylab("d' (Condition - Novel)") +
  ggtitle("Memory Performance (Minus Novel) Exp 6") +
  theme_bw()
print(p6_minus_d)

# ---- Plot B2: Hits minus Novel ----
p6_minus_h <- ggplot() +
  geom_point(data=plot_minus_h6, aes(x=x_plot, y=Hits, color=contrast), size=3, alpha=0.7) +
  geom_line(data=plot_minus_h6 %>% arrange(nam_unn, participant, contrast),
            aes(x=x_plot, y=Hits, group=interaction(participant, nam_unn)),
            alpha=0.35, color="grey") +
  geom_point(data=SE_minus_h6, aes(x=x_mean, y=Hits),
             inherit.aes=FALSE, shape=18, size=4, color="black") +
  geom_errorbar(data=SE_minus_h6, aes(x=x_mean, ymin=Hits-se, ymax=Hits+se),
                inherit.aes=FALSE, width=0.12, color="black") +
  scale_x_continuous(breaks=1:length(levels(d6$nam_unn)), labels=stim_labels) +
  scale_color_manual(name="Contrast", values=contrast_cols) +
  ylab("Hits (Condition - Novel)") +
  ggtitle("Hits (Minus Novel) Exp 6") +
  theme_bw()
print(p6_minus_h)

# ---- Plot B3: FAs minus Novel (auto ylim) ----
fa_top6m <- max(plot_minus_fa6$FAs, SE_minus_fa6$FAs + SE_minus_fa6$se, na.rm=TRUE) * 1.05
fa_bot6m <- min(0, min(plot_minus_fa6$FAs, na.rm=TRUE))

p6_minus_fa <- ggplot() +
  geom_point(data=plot_minus_fa6, aes(x=x_plot, y=FAs, color=contrast), size=3, alpha=0.7) +
  geom_line(data=plot_minus_fa6 %>% arrange(nam_unn, participant, contrast),
            aes(x=x_plot, y=FAs, group=interaction(participant, nam_unn)),
            alpha=0.35, color="grey") +
  geom_point(data=SE_minus_fa6, aes(x=x_mean, y=FAs),
             inherit.aes=FALSE, shape=18, size=4, color="black") +
  geom_errorbar(data=SE_minus_fa6, aes(x=x_mean, ymin=FAs-se, ymax=FAs+se),
                inherit.aes=FALSE, width=0.12, color="black") +
  scale_x_continuous(breaks=1:length(levels(d6$nam_unn)), labels=stim_labels) +
  scale_color_manual(name="Contrast", values=contrast_cols) +
  coord_cartesian(ylim=c(fa_bot6m, fa_top6m)) +
  ylab("FAs (Condition - Novel)") +
  ggtitle("FAs (Minus Novel) Exp 6") +
  theme_bw()
print(p6_minus_fa)
...
write.csv(dprimeTable, file=paste("C:/Users/jgove/OneDrive/Desktop/dPrimeTable-4-1.csv", sep=""),row.names = F)
